...
export function formatarMascarar({ antigo, valor, mascara, padrao }) {
  ...
  const resultado = valores.reduce(
    (acumulado, atual) => acumulado.replace(caracterMascara, atual),
    mascara,
  );

  if (resultado.indexOf(caracterMascara) < 0)
    return resultado.slice(0, mascara.length);
